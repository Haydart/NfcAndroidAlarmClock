...
import com.example.radek.nfc_test.Settings;
...
public class ExpandingListView extends ListView {
    ...
    private Handler animationEndHandler = new Handler();
    private List<View> mViewsToDraw = new ArrayList<View>();
    private int[] mTranslate;
    private boolean bIsListInteractable = true;
    ...
    public void expandView(final View view) {
        if(bIsListInteractable){
            bIsListInteractable = false;
            final ExpandableListItem viewObject = (ExpandableListItem)getItemAtPosition(getPositionForView
                    (view));

            animationEndHandler.postDelayed(new Runnable() { // OnAnimationEnd is not reliable under load
                @Override
                public void run() {
                    bIsListInteractable = true;
                    viewObject.setExpanded(true); //done manually at list adapter to prevent bugs
                    setEnabled(true);
                    setClickable(true);
            ...
            }, Settings.LIST_ANIMATION_DURATION);
        ...
        * determined, but still have not been rendered onto the screen.*/
            ...
            Log.d("LIST IS NOT", "INTERACTABLE AT THIS TIME!");
        }
    }
